#pragma once
#define LI_FN(name) ::li::detail::lazy_function<LAZY_IMPORTER_KHASH(#name), decltype(&name)>().safe_cached()
...
#define LI_FN2(name) ::li::detail::lazy_function<LAZY_IMPORTER_KHASH(#name), decltype(&name)>()

#define LI_FN_CUSTOM_DEF(name, def) ::li::detail::lazy_function<LAZY_IMPORTER_KHASH(#name), def>()
...
#define LAZY_IMPORTER_KHASH(str)                       \
    ::li::detail::khash(str, ::li::detail::khash_impl( \
                                 __TIME__ __DATE__ LAZY_IMPORTER_STRINGIZE_EXPAND(__LINE__) LAZY_IMPORTER_STRINGIZE_EXPAND(__COUNTER__), 2166136261))
...
namespace li {
    namespace detail {
        ...
        LAZY_IMPORTER_FORCEINLINE constexpr unsigned hash_single(unsigned value, char c) noexcept {
            return static_cast<unsigned int>((value ^ ((CaseSensitive && c >= 'A' && c <= 'Z') ? (c | (1 << 5)) : c)) *
