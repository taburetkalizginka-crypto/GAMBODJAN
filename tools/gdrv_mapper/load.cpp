	...
	)
{
	...
	Printf(L"[!] ciValidateImageHeaderEntry: %p\n", w.ciValidateImageHeaderEntry);
	Printf(L"[!] zwFlushInstructionCache : %p\n", w.zwFlushInstructionCache);
	...
	Printf(L"[*] OriginalRevokedCallback : %p\n", dataR);
	...
	if (!NT_SUCCESS(Status))
		return Status;
	Printf(L"[*] Loaded Driver with (0) Errors.\n");
	...
	if (!NT_SUCCESS(Status))
		Printf(L"NtDeviceIoControlFile(IOCTL_GIO_MEMCPY) *WRITE* failed: error %08X\n", Status);
	Printf(L"[*] Restored RevokedCallback \n");
