...
void CPanel2D__OnMouseMove( CPanel2D* rcx, float flMouseX, float flMouseY ) {
	...
	if ( rcx == panorama_gui.main_panel->panel2d_as( ) ) {
		if ( GetKeyState( VK_LBUTTON ) & 0x8000 ) {
			...
			const float uixp = static_cast<float>( p.x ) / uiw * 100.f;
			const float uiyp = static_cast<float>( p.y ) / uih * 100.f;

			char buf[128];
			snprintf( buf, sizeof( buf ), "position: %.1f%% %.1f%% 0;", uixp, uiyp );
			panorama_gui.main_panel->SetStyle( buf );
		}
	}
}
