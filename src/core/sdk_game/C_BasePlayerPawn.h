...
public:
	...
	uint32_t GetFOV( ) { return Member<uint32_t>( schema::CPlayer_CameraServices::m_iFOV ); }
	void SetFOV( uint32_t fov ) { Member<uint32_t>( schema::CPlayer_CameraServices::m_iFOV ) = fov; }
	void SetFOVStart( uint32_t fov ) { Member<uint32_t>( schema::CPlayer_CameraServices::m_iFOVStart ) = fov; }
	void SetFOVRate( float rate ) { Member<float>( schema::CPlayer_CameraServices::m_flFOVRate ) = rate; }
};
