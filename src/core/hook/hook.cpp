...
void* hook::functions::PostReceivedNetMessage( INetChannel* rcx, CNetworkSerializerPB* rdx, google::protobuf::Message* r8, NetChannelBufType_t* r9 ) {
	...
	if ( IEngineClient::get( ).IsInGame( ) ) {
		...
		features::camera_hack.reset_camera( );
...
long hook::functions::Present( IDXGISwapChain* pSwapchain, UINT SyncInterval, UINT Flags ) {
	...
	// Sync menu::state → panorama_gui / pGui settings
	panorama_gui.draw_health = menu::state.esp_health;
	panorama_gui.draw_mana_bar = menu::state.esp_mana;
	panorama_gui.draw_abilities = menu::state.esp_abilities;
	pGui->mouse_distance = menu::state.mouse_distance;
	pGui->fog = !menu::state.fog_enabled;
	pGui->auto_accept = menu::state.auto_accept;
	pGui->accept_in_last_moment = menu::state.accept_last_moment;
	pGui->auto_accept_delay = menu::state.auto_accept_delay;
	pGui->camera_step = menu::state.camera_step;

	// Sync camera hack settings from menu
	features::camera_hack.set_camera_hack_enabled( menu::state.fov_enabled );
	features::camera_hack.set_camera_distance( menu::state.fov_value );

	// Apply camera distance every frame (must be in Present to override game's own distance)
	if ( IEngineClient::get( ).IsInGame( ) ) {
		features::camera_hack.apply_camera_hack( );
	}

	// Render the GAMBODJAN menu
	pGui->Render( );
	...
	if ( IEngineClient::get( ).IsInGame( ) && g_pGameRules && ( g_pGameRules->GetGameState( ) == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS || g_pGameRules->GetGameState( ) == DOTA_GAMERULES_STATE_PRE_GAME ) ) {
		...
		if ( menu::state.esp_health )
			features::hero_bar.draw_health( menu::state.esp_mana );

		if ( menu::state.esp_abilities )
			features::hero_bar.draw_abilities_and_items( );
	}
...
void hook::functions::OnMouseWheeled( CDOTA_Camera* rcx, int delta ) {
	...
	if ( menu::state.mouse_distance && !pGui->show && IEngineClient::get( ).IsInGame( ) ) {
		features::camera_hack.on_mouse_wheeled( rcx, delta );

		// Sync distance back to menu slider
		menu::state.fov_value = features::camera_hack.get_distance( );
